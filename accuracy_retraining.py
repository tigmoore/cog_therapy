...
def main(seed: int):
    ...
    # analyze initial metrics: class accuracies 
    class_accuracies, difficulties = utils.compute_class_accuracies(model=model, dataloader=test_loader, device=device)
    ...
    for i in range(len(percentage)):
        ...
        decay_accuracies, decay_difficulties = utils.compute_class_accuracies(model=model, dataloader=test_loader, device=device)

        # only save one seed of model to do geometric comparisions 
        if seed == 42:
            torch.save(model.state_dict(), f'saved_models/injured_models/accuracy_retraining/3000/3000_damaged_{i}.pth')
        ######################################################################
        # build new retraining data according to inverse of class accuracies
        ###################################################################
        imbalanced_val_sampler = utils.create_imbalanced_sampler(train_dst, valid_index, class_accuracies=decay_accuracies, num_samples=3000)
        imbalanced_val_loader = torch.utils.data.DataLoader(train_dst, batch_size=batch_size, sampler=imbalanced_val_sampler, num_workers=0)

        #####################################################################
        utils.train_model(model, imbalanced_val_loader, test_loader, num_epochs, test_noise_dict=test_noise_dict, w_bkup=w_bkup, lr=0.01) 
        ...
        if seed == 42:
            torch.save(model.state_dict(), f'saved_models/injured_models/accuracy_retraining/3000/3000_retrained_{i}.pth')
